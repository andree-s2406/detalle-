...
import { escapeHtml, colorDot, colorSlug } from '../components/Formatter.js';
...
function _renderColorsGrid() {
  ...
  const cards = allColors.map(c => `
    <div class="color-card-item">
      <div class="color-swatch-box color-swatch-${colorSlug(c.nombre)}"></div>
      <div class="color-card-info">
        <div class="font-semibold text-md">${escapeHtml(c.nombre)}</div>
        <div class="text-xs text-muted mt-xs">${c.activo ? '<span class="text-success font-semibold">● Activo</span>' : '<span class="text-muted">○ Inactivo</span>'}</div>
      </div>
      <div class="flex gap-sm">
        ...
        <button class="btn btn-ghost btn-icon btn-sm" title="${c.activo ? 'Desactivar' : 'Activar'}" onclick="window._toggleColor('${c.id}')">
          <span class="${c.activo ? 'text-danger' : 'text-success'} flex-center">
            ${icon('power', '', 14)}
          </span>
        </button>
  ...
  container.innerHTML = `
    ...
    <div class="text-muted text-sm mt-sm">${allColors.length} color(es) registrado(s)</div>
  `;
}
